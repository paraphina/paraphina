...
roadmap gates accept complete non-live evidence. The current Phase 5.1p
boundary recovered all previously missing filled-order source-tick horizons,
aligned historical Lighter active-order snapshots without treating them as full
sendTx/REST pressure, and exhausted the quarantined historical Lighter native
trade join attempt with `0 / 125` exact canonical recoveries. Blockers remain:
native-limit pressure is still partial, venue-native maker/taker evidence is
incomplete for `287` filled rows, calibration buckets are sparse, and
observed-only selection-bias quarantine policy remains unresolved.
...
> **[STATUS: Partial]** Phase A "A1" vertical slice is **Implemented**: Monte Carlo tail-risk metrics (VaR, CVaR, quantiles, Wilson CI for kill probability) are emitted in `mc_summary.json` (schema_version=2). Pareto tuning harness scaffold exists at `batch_runs/exp_phase_a_pareto_mc.py` with risk-tier budgets. **Phase A-2 (B2) is Partial**: Adversarial search harness at `batch_runs/exp_phase_a_adversarial_search.py` with failure seed extraction and regression suite generation (`scenarios/suites/adversarial_regression_v1.yaml`). CI gate at `.github/workflows/adversarial_regression.yml`. **Phase B (Confidence-Aware Statistical Gating) is Implemented**: Bootstrap-based confidence intervals and tri-state decision model (PROMOTE/HOLD/REJECT/ERROR) for promotion decisions (`batch_runs/phase_b/`). PROMOTE requires candidate to be provably better; HOLD occurs when guardrails pass but candidate is not provably better (common for smoke runs with small sample sizes); REJECT occurs when candidate is provably worse. See `docs/PHASE_B_CONFIDENCE_GATING.md`. Remaining methods (B3-B5) are **Planned (see ROADMAP.md Phase A)**. The batch harness (`batch_runs/`) provides foundational infrastructure.
...
> **[STATUS: Implemented]** Phase B implements a **tri-state decision model**: PROMOTE (candidate provably better), HOLD (not worse but not provably better), REJECT (provably worse), ERROR (data issues). Guardrails check non-regression; promotion criteria check superiority. Smoke runs commonly yield HOLD due to small sample sizes—this is expected and healthy. See `docs/PHASE_B_CONFIDENCE_GATING.md`.

A candidate configuration/policy is promoted only if it:
- passes all guardrails (not provably worse on any metric),
- passes all promotion criteria (provably better on PnL and drawdown at configured alpha),
