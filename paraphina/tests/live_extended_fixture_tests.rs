#[cfg(all(feature = "live", feature = "live_extended"))]
mod tests {
    ...
    #[test]
    fn extended_fixture_paper_run_is_healthy() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|e| e.into_inner());
        let temp_dir = tempdir().expect("tempdir");
        let out_dir = temp_dir.path().join("extended_fixture_paper");
        ...
        let output = Command::new(bin_path)
            ...
            .env("EXTENDED_FIXTURE_DIR", fixture_dir.to_string_lossy().to_string())
            ...
            .output()
            .expect("failed to run paraphina_live in extended fixture mode");
