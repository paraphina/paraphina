...
impl Config {
    ...
    ///   - PARAPHINA_MM_VENUE_ROLE_<VENUE> (fill|anchor|noise)
        ...
        for venue in &cfg.venues {
            ...
            if let Ok(raw) = env::var(&venue_role_key) {
                match MmVenueRole::parse(&raw) {
                    ...
                    None => {
                        eprintln!(
                            "[config] WARN: could not parse {} = {:?} as fill|anchor|noise; leaving {} on default fill role",
                            venue_role_key,
                            raw,
                            venue.id
                        );
...
mod tests {
    ...
    fn mm_live_tuning_envs_override_defaults() {
        ...
        env::set_var(HL_ROLE_KEY, "anchor");
        ...
        assert_eq!(cfg.mm.venue_role_for("hyperliquid"), MmVenueRole::Anchor);
