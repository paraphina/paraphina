...
use crate::telemetry::{
    ...
    TelemetryInputs, TelemetrySink, TelemetrySinkHandle,
};
#[cfg(feature = "event_log")]
            ...
            match ordered.event {
                ...
                CanonicalEvent::Execution(event) => {
                    ...
                    if let super::types::ExecutionEvent::OrderSnapshot(snapshot) = event {
                        ...
                        let (core_events, fills) =
                            infer_fills_from_order_snapshot(cfg, &mut state, &snapshot, now_ms);
...
fn apply_priority_response_events(
    ...
    for event in events {
        ...
        if let super::types::ExecutionEvent::OrderSnapshot(snapshot) = event {
            let (core_events, fills) =
                infer_fills_from_order_snapshot(cfg, state, &snapshot, now_ms);
            if !core_events.is_empty() {
                tick_exec_events.extend(core_events.iter().cloned());
            }
            if !fills.is_empty() {
