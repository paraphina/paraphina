...
fn v2_live_canary_admission_preflight_check(
    ...
    Some(PreflightCheck {
        ...
        details: format!(
            "approved={} canary_mode={} profile_metadata={} max_position={} max_gross_position={} max_abs_venue_position={} max_open_orders={} post_only={} reduce_only_not_enforced={} baseline_hedge_authority_ack={} exit_cancel_all={} exit_cancel_all_sweep_all_venues={} exit_position_flatten={} venue_coverage_replacements_disabled={} venue_coverage_baseline_hedge_disabled={} venue_coverage_probe_approved={} venue_coverage_probe_venues_present={} ranked_execution_venues_present={} pair_edge={} order_intent={} fast_hedge_disabled={} require_phase51={}",
            ...
            gate_state.live_canary_venue_coverage_baseline_hedge_disabled,
...
mod tests {
    ...
    fn v2_live_ranked_admission_micro_canary_profile_has_required_caps() {
        ...
        assert_eq!(settings.soft_max_abs_venue_position_tao, Some(0.02));
        assert_eq!(
            canary
                .venue
                .as_ref()
                .and_then(|venue| venue.base_order_size),
            Some(0.01)
        );
        assert_eq!(
            canary.venue.as_ref().and_then(|venue| venue.max_order_size),
            Some(0.01)
        );
        assert_eq!(
            canary.venue.as_ref().and_then(|venue| venue.lot_size_tao),
            Some(0.005)
        );
    ...
    fn with_v2_live_canary_preflight_env<R>(f: impl FnOnce() -> R) -> R {
        ...
        let _venue_coverage_baseline_hedge_disabled =
            EnvVarGuard::new("PARAPHINA_V2_LIVE_CANARY_VENUE_COVERAGE_DISABLE_BASELINE_HEDGE");
    ...
    fn v2_live_canary_admission_preflight_accepts_required_runtime_gates() {
        with_v2_live_canary_preflight_env(|| {
            ...
            assert!(check
                .details
                .contains("venue_coverage_baseline_hedge_disabled=true"));
            assert!(check
                .details
    ...
    #[test]
    fn v2_live_canary_admission_preflight_rejects_venue_coverage_without_cleanup_gates() {
        with_v2_live_canary_preflight_env(|| {
            let mut cfg = v2_live_canary_admission_config();
            cfg.v2_shadow.live_canary_venue_coverage_probe_approved = true;
            cfg.v2_shadow.live_canary_venue_coverage_probe_venues = vec!["extended".to_string()];
    ...
    #[test]
    fn v2_live_canary_admission_preflight_rejects_venue_coverage_without_baseline_hedge_gate() {
        with_v2_live_canary_preflight_env(|| {
            let mut cfg = v2_live_canary_admission_config();
            cfg.v2_shadow.live_canary_venue_coverage_probe_approved = true;
            cfg.v2_shadow.live_canary_venue_coverage_probe_venues = vec!["paradex".to_string()];

            std::env::remove_var("PARAPHINA_V2_LIVE_CANARY_VENUE_COVERAGE_DISABLE_BASELINE_HEDGE");
            let check = super::v2_live_canary_admission_preflight_check(TradeMode::Live, &cfg)
                .expect("preflight check should run");
            ...
            assert!(check
                .details
                .contains("venue_coverage_baseline_hedge_disabled=false"));
        });
    }
