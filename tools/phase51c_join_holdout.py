    ...
    *,
    ...
    for seq, fill in enumerate(fills, start=1):
        ...
        record.update({
            ...
            "order_label_seq": order.get("label_seq") if order else None,
            "order_source_line": order.get("source_line") if order else None,
            "order_source_t": order.get("source_t") if order else None,
            "order_source_order_index": order.get("source_order_index") if order else None,
            "order_id_hash": order.get("order_id_hash") if order else fill.get("order_id_hash"),
            "client_order_id_hash": order.get("client_order_id_hash") if order else fill.get("client_order_id_hash"),
