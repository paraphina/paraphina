...
import json
import sys
from dataclasses import asdict  # kept for future use if needed
...
# ---------------------------------------------------------------------
# Make the batch_runs directory importable as a plain module root
# (avoid name clash with tools/batch_runs.py).
# ---------------------------------------------------------------------
ROOT = Path(__file__).resolve().parents[1]
BATCH_RUNS_DIR = ROOT / "batch_runs"
if str(BATCH_RUNS_DIR) not in sys.path:
    sys.path.insert(0, str(BATCH_RUNS_DIR))

from ts_metrics import load_telemetry_jsonl
from profile_centres import get_profile_centre
...
def compute_run_metrics(df: pd.DataFrame) -> dict:
    ...
    # Treat non-finite points as “no change” for drawdown
    finite_mask = np.isfinite(pnl_values)
    if finite_mask.any():
        pnl_valid = pnl_values[finite_mask]
        cummx = np.maximum.accumulate(pnl_valid)
        dd = cummx - pnl_valid
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Output JSONL dataset to append to (default: research_run_001.jsonl).",
    )
    parser.add_argument(
        "--notes",
        type=str,
