...
class TestV2TelemetryOrderPathCoverageValidator(unittest.TestCase):
    ...
    def test_missing_sweep_all_preflight_gate_holds(self):
        report = self.evaluate(preflight=preflight_text().replace("exit_cancel_all_sweep_all_venues=true ", ""))

        self.assertEqual(report["validation_status"], "HOLD")
        self.assertTrue(
            any(reason.startswith("preflight_missing:exit_cancel_all_sweep_all_venues=true") for reason in report["validation_reasons"])
        )
