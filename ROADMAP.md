...
| current status | `specified_holding_nonlive`; `docs/V2_SPECIFICATION.md` is the active target spec, Phase 5.1n is the current evidence boundary, the filled-horizon source-tick blocker is cleared, historical Lighter active-order snapshots are event-time aligned for many labels but sendTx/REST pressure remains unobserved, maker/taker native role evidence remains incomplete, and no V2 live/canary/model-training/EV-admission promotion is authorized. |
| next action | Close the highest-leverage remaining non-live evidence gaps: venue-native maker/taker completeness across all filled venues, event-time sendTx/REST/native-limit pressure instrumentation where available, sparse-bucket/selection-bias holds, then rerun the recovered 5.1h/5.1i evidence chain before any calibrated EV shadow review. |
| required evidence | Repo-owned V2 spec, Phase 5.1n recovered feature matrix, filled-horizon source-key recovery evidence pack, Lighter event-time active-order alignment evidence, venue-native maker/taker evidence, schema v2 validation, deterministic replay outputs, and docs integrity output. |
