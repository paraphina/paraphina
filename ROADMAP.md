...
| next single move | Find a new safe Lighter observed source or validated redacted linkage sidecar that can connect native trade rows to the current canonical target groups, and separately find a non-live-authorized source for event-time sendTx plus REST-or-weighted limit/remaining pressure. Continue Aster/Extended/Paradex read-only recovery only when it can improve target linkage, not by refetching already no-target-matching rows. Do not infer missing Lighter pressure from GET-only caps, empty headers, account tiers, or documentation-only limits. |
