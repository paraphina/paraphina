...
| current status | `specified_holding_nonlive`; `docs/V2_SPECIFICATION.md` is the active target spec, Phase 5.1s is the active local source-staging gate in front of Phase 5.1r/5.1q, and Phase 5.1p remains the last completed historical evidence boundary. The first 5.1s run over existing Lighter local snapshots staged `405` rows and stripped `3500` raw identifier fields, but found `0` join-key rows; downstream 5.1r still recovers `0 / 287` native-role targets and `0 / 3132` Lighter native-limit targets. No V2 live/canary/model-training/EV-admission promotion is authorized. |
