...
| current status | `specified_holding_nonlive`; `docs/V2_SPECIFICATION.md` is the active target spec. Phase 5.1u emits the exact forward capture target manifest from the canonical P-fill labels: `287` missing all-five venue-native role targets (`aster=113`, `extended=28`, `hyperliquid=6`, `lighter=125`, `paradex=15`) and `3132` Lighter event-time native-limit pressure targets. Phase 5.1w now emits the exact operator request pack and manifest skeleton for the six required sanitized local source files. A 2026-05-04 read-only private source attempt succeeded for Lighter account/native-limit and trade-backfill artifacts only: `runs/phase51b_lighter_account_native_limits/PHASE51B-LIGHTER-ACCOUNT-NATIVE-LIMITS_20260504T110402Z` and `runs/phase51c_lighter_trade_backfill/PHASE51C-LIGHTER-TRADE-BACKFILL-20260504T110416Z`. It remains `HOLD` for all-five Phase 5.1w because Aster, Extended, Hyperliquid, and Paradex still lack safe repo-owned read-only native-role collectors, and Lighter event-time native-limit pressure remains incomplete. No V2 live/canary/model-training/EV-admission promotion is authorized. |
| next action | Implement or add safe repo-owned read-only native-role collectors/parsers for Aster `ORDER_TRADE_UPDATE.m`, Extended `isTaker`, Hyperliquid `crossed`, and Paradex `liquidity`, and complete Lighter event-time active-order/sendTx/REST-or-weighted pressure linkage. Then build the six-file Phase 5.1w local capture bundle and rerun Phase 5.1v against the Phase 5.1u target manifest. If 5.1v emits `generated_phase51s_manifest_ready=true`, run Phase 5.1s -> 5.1r -> 5.1q -> 5.1n -> 5.1h -> 5.1i, and only after blocker reduction address sparse-bucket/selection-bias holds before any calibrated EV shadow review. |
