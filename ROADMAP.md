...
| current status | `specified_holding_nonlive`; `docs/V2_SPECIFICATION.md` is the active target spec, Phase 5.1r is the active forward native source-acquisition layer feeding Phase 5.1q, and Phase 5.1p remains the last completed historical evidence boundary. The 5.1r baseline no-source run is redaction-safe but clears no blocker: `287` filled rows still lack exact venue-native maker/taker source and `3132` Lighter rows still lack event-time native-limit pressure source. No V2 live/canary/model-training/EV-admission promotion is authorized. |
| next action | Capture or locate real read-only venue-native snapshots, normalize them through Phase 5.1r, feed sanitized outputs into Phase 5.1q, rerun Phase 5.1n/5.1h/5.1i, then address sparse-bucket/selection-bias holds before any calibrated EV shadow review. |
| required evidence | Repo-owned V2 spec, Phase 5.1p recovered feature matrix, Phase 5.1r source-acquisition artifacts, Phase 5.1q forward native-evidence artifacts, filled-horizon source-key recovery evidence pack, Lighter event-time active-order alignment evidence, exact canonical venue-native maker/taker evidence, schema v2 validation, deterministic replay outputs, and docs integrity output. |
