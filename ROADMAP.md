...
| next single move | Fill `runs/phase51z_source_link_request_pack/PHASE51Z-LIGHTER-SOURCE-LINK-REQUEST-PACK-HOLD-20260505T000000Z/source_links.proposed.empty.jsonl` with a validated redacted source-link sidecar that maps `source_record_sha256` to `canonical_group_id` or `order_key`, then rerun Phase 5.1v against the request-pack candidate manifest. If no valid sidecar can be produced, attempt a bounded GET-only target-window Lighter `/api/v1/trades` capture only as a diagnostic. Separately find a non-live-authorized source for event-time sendTx plus REST-or-weighted limit/remaining pressure. Do not infer missing Lighter pressure from GET-only caps, empty headers, account tiers, or documentation-only limits. |
