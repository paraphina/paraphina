...
| current status | `specified_holding_nonlive`; `docs/V2_SPECIFICATION.md` is the active target spec. Phase 5.1u now emits the exact forward capture target manifest from the canonical P-fill labels: `287` missing all-five venue-native role targets (`aster=113`, `extended=28`, `hyperliquid=6`, `lighter=125`, `paradex=15`) and `3132` Lighter event-time native-limit pressure targets. Phase 5.1u is HOLD-only and clears no blocker by itself. Current matrix blockers remain Lighter native-limit pressure, maker/taker completeness, sparse buckets, and observed-only selection bias. No V2 live/canary/model-training/EV-admission promotion is authorized. |
| next action | Execute a fresh read-only forward source-capture pilot against the Phase 5.1u target manifest: capture all-five venue-native maker/taker source rows and Lighter event-time active-order/sendTx/REST-or-weighted-request pressure with canonical group/order-key linkage, or with Phase 5.1t-generated redacted source-link sidecars staged through Phase 5.1s. Then rerun Phase 5.1s -> 5.1r -> 5.1q -> 5.1n -> 5.1h -> 5.1i, and only after blocker reduction address sparse-bucket/selection-bias holds before any calibrated EV shadow review. |
