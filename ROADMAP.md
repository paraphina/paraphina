...
| latest Phase 5.1z evidence | `tools/phase51z_readonly_native_role_capture.py` adds a HOLD-only read-only private-source capture/sanitizer for Aster, Extended, Lighter local rows, and Paradex. The authorized 2026-05-04 run `runs/phase51z_readonly_native_role_capture/PHASE51Z-READONLY-NATIVE-ROLE-CAPTURE-20260504T000000Z` fetched read-only Aster (`268` rows), Extended (`1601` rows), and Paradex (`30` rows), skipped Lighter network capture in favor of existing 5.1b/5.1c local sources, emitted `67` sanitized target-linked native-role rows (`aster=39`, `extended=21`, `paradex=7`), and passed raw-identifier/secret redaction. Combined with the existing Phase 5.1x Hyperliquid source, Phase 5.1v now recognizes `73 / 287` native-role targets ready and still `0 / 3132` Lighter native-limit targets ready at `runs/phase51v_forward_capture_bundle_readiness/PHASE51V-COMBINED-PHASE51Z-HYPERLIQUID-HOLD-20260504T000000Z`. Existing retained Lighter backfills reprocessed through Phase 5.1z recovered `0` additional target-linked Lighter native-role rows, so the retained Lighter source path is exhausted for current target recovery. |
...
| next single move | Continue Phase 5.1z-style read-only source recovery for the remaining Aster/Extended/Paradex unmatched target groups and identify a safe observed Lighter source that contains target-linked native role plus complete event-time active-order/sendTx/REST-or-weighted request pressure. Do not attempt to infer missing Lighter pressure from GET-only caps or headers. |
