...
| next action | Obtain or produce materially new directly target-linkable private/native source rows and run them through Phase 5.1aj, or obtain a validated redacted source-link mapping for `runs/phase51z_source_link_request_pack/PHASE51Z-CURRENT-TARGET-WIDE-SOURCE-LINK-REQUEST-PACK-HOLD-20260505T000000Z` and materialize it through Phase 5.1ad. Separately obtain non-live-authorized sanitized Lighter event-time pressure rows containing active-order headroom, sendTx limit/remaining, REST-or-weighted limit/remaining, and event-time alignment, and stage them through Phase 5.1ab. Run Phase 5.1ak as the final wrapper so accepted artifacts are composed through Phase 5.1ae and validated through Phase 5.1v. If Phase 5.1ak remains incomplete, stop retrospective local mining and move only to a board-documented forward-refresh target pack with event-time source truth. |
| next single move | Do not repeat the same local retrieval audit, seven-day or expanded Aster/Extended/Paradex read-only native-role capture, GET-only target-window Lighter diagnostic/local sanitizer, Lighter signer-venv current account-limit capture, Lighter WS `account_all`/`account_all_trades` snapshot, Lighter inactive-order/export bridge audit, Lighter Explorer log/tx bridge diagnostic, Extended/Paradex order-history bridge diagnostic, or Phase 5.1aj normalization of the same local artifacts unless account activity, target window, source semantics, auth material, retained source rows, or local artifacts change materially. The next move is to obtain materially new directly target-linkable private/native rows for Phase 5.1aj, obtain a deterministic redacted source-link mapping for Phase 5.1ad, or obtain sanitized Lighter event-time pressure rows for Phase 5.1ab, then run Phase 5.1ak. Do not infer source links from time/price/size collisions, and do not infer missing Lighter pressure from GET-only caps, current account snapshots, empty headers, account tiers, WebSocket account snapshots with empty trades, inactive-order target hash overlap without native-source overlap, trade-export CSV rows without order/client identifiers, Explorer logs/txs that do not overlap current request-pack source hashes and target hashes, Extended/Paradex order-history rows that do not bridge request-pack native source hashes to target hashes, runtime `rate_limit` config strings, or documentation-only limits. |
