...
| next single move | Do not repeat the same GET-only target-window Lighter diagnostic, and do not repeat the same Lighter WS `account_all`/`account_all_trades` snapshot unless account activity, target window, or source semantics change. The next move is to obtain or produce a deterministic redacted source-link mapping for `runs/phase51z_source_link_request_pack/PHASE51Z-CURRENT-TARGET-WIDE-SOURCE-LINK-REQUEST-PACK-HOLD-20260505T000000Z`, materialize it through Phase 5.1ad, and rerun Phase 5.1v against the materialized candidate manifest; alternatively capture a different non-live authorized source that is directly target-linkable to the current Phase 5.1u hashes. Separately obtain non-live-authorized sanitized Lighter event-time pressure rows containing active-order headroom, sendTx limit/remaining, REST-or-weighted limit/remaining, and event-time alignment, stage them through Phase 5.1ab, and rerun Phase 5.1v. Do not infer missing Lighter pressure from GET-only caps, empty headers, account tiers, WebSocket account snapshots with empty trades, or documentation-only limits. |
