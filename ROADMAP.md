...
| next single move | Prioritize a Lighter-only safe observed source path that contains target-linked native role plus complete event-time active-order/sendTx/REST-or-weighted request pressure. Continue Aster/Extended/Paradex read-only recovery only when it can improve target linkage, not by refetching already no-target-matching rows. Do not infer missing Lighter pressure from GET-only caps or headers. |
