...
| current status | `specified_holding_nonlive`; `docs/V2_SPECIFICATION.md` is the active target spec, Phase 5.1q is the active forward-evidence gate, and Phase 5.1p remains the last completed historical evidence boundary. The filled-horizon source-tick blocker is cleared; historical Lighter active-order snapshots are event-time aligned for many labels but sendTx/REST pressure remains unobserved; all retained Lighter native trade backfills were quarantined and hashed into a canonical join attempt with `0 / 125` recoveries; existing maker/taker source artifacts still do not provide exact canonical joins for `287` filled rows. No V2 live/canary/model-training/EV-admission promotion is authorized. |
