...
| current status | `specified_holding_nonlive`; `docs/V2_SPECIFICATION.md` is the active target spec. Phase 5.1u emits the exact forward capture target manifest from the canonical P-fill labels: `287` missing all-five venue-native role targets (`aster=113`, `extended=28`, `hyperliquid=6`, `lighter=125`, `paradex=15`) and `3132` Lighter event-time native-limit pressure targets. Phase 5.1w now emits the exact operator request pack and manifest skeleton for the six required sanitized local source files. Phase 5.1v validates whether a supplied local sanitized forward capture bundle is ready for Phase 5.1s; the baseline 5.1v run against the 5.1u placeholder template remains `HOLD`: `0 / 287` native-role targets ready, `0 / 3132` Lighter native-limit targets ready, `PLACEHOLDER_PATH` source status, and no blocker cleared. Current matrix blockers remain Lighter native-limit pressure, maker/taker completeness, sparse buckets, and observed-only selection bias. No V2 live/canary/model-training/EV-admission promotion is authorized. |
| next action | Use the Phase 5.1w request pack to acquire or produce the six sanitized local read-only all-five forward capture files, then run Phase 5.1v against the Phase 5.1u target manifest. The bundle must contain all-five venue-native maker/taker source rows and Lighter event-time active-order/sendTx/REST-or-weighted-request pressure with canonical group/order-key linkage, or with Phase 5.1t-generated redacted source-link sidecars staged through Phase 5.1s. If 5.1v emits `generated_phase51s_manifest_ready=true`, run Phase 5.1s -> 5.1r -> 5.1q -> 5.1n -> 5.1h -> 5.1i, and only after blocker reduction address sparse-bucket/selection-bias holds before any calibrated EV shadow review. |
