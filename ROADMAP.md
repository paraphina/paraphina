...
| next single move | Build or acquire a validated redacted source-link sidecar for the `531` sanitized unlinked Lighter source rows, or capture a new safe target-window Lighter observed source whose native trade rows directly match current canonical target groups. Separately find a non-live-authorized source for event-time sendTx plus REST-or-weighted limit/remaining pressure. Continue Aster/Extended/Paradex read-only recovery only when it can improve target linkage, not by refetching already no-target-matching rows. Do not infer missing Lighter pressure from GET-only caps, empty headers, account tiers, or documentation-only limits. |
