...
| current blocker | Phase 5.1 remains `HOLD`: `214 / 287` native-role targets are still missing after Phase 5.1z plus Hyperliquid (`aster=74`, `extended=7`, `lighter=125`, `paradex=8`), and Lighter event-time native-limit pressure remains `0 / 3132`. The all-venue source-link request pack makes the native-role linkage blocker reviewable but does not clear it: a validated redacted sidecar is still required before unlinked hashes can mark targets ready. The retained Lighter trade-backfill source path is exhausted for direct target recovery even after target-link hygiene and side-order-ID matching; the captured Lighter WS account-source surface is redaction-safe but produced `0` trade rows. Phase 5.1ab makes future sanitized Lighter pressure rows directly checkable by Phase 5.1v but does not create those observed rows. No V2 live/canary/model-training/EV-admission promotion is authorized. |
